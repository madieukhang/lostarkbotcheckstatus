...
- Evidence dropdown on `/la-check` slash response and the auto-check listener reply. Mirrors `/la-list view`'s "View evidence for..." design: surfaces every flagged row whose blacklist/whitelist/watchlist entry has an attached image, options are labelled with the entry's list-type emoji + reason snippet, value encodes `<listType>:<_id>` so the select handler resolves unambiguously across types and scopes (same encoding as `/la-evidence`). Selecting an option replies ephemerally with the same `buildEvidenceEmbed` renderer used by `/la-list view` and `/la-search` so all four evidence surfaces look identical. Officer-only "Added by" footer follows the existing gate. Sits below the Quick-Add row when both are present.
