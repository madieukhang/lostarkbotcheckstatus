...
### Fixed
- Auto-check / `/la-check` no longer folds cached `RosterSnapshot` rows across distinct diacritic variants before enrichment. Observed case: Bible has both `Aüreliá` and `Aürélià`; the bot now treats inexact snapshot matches as untrusted and lets Bible search / worker exact-match the character before rendering. 136/136 green.
