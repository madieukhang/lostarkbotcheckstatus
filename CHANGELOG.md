...
### Added
- Auto-check text input now documents and regression-tests both multi-name forms used by the roster team: `check NameOne NameTwo` and `check NameOne, NameTwo` (up to 8 unique valid names). Check result cards now expose a **View details** dropdown for every blacklist/watchlist/whitelist hit, even when no evidence image was attached; the detail card still shows reason, raid, list, age, tracked alts, logs, and the existing officer/senior-only **Added by** field, while distinguishing “no image attached” from an expired evidence link.
...
- Details dropdown on `/la-check` slash responses and auto-check listener replies. It surfaces every blacklist/whitelist/watchlist hit, including entries without an attached image, and de-duplicates multiple checked alts that resolve to the same entry. Options keep the `<listType>:<_id>` encoding so the select handler resolves unambiguously across types and scopes (same encoding as `/la-evidence`). Selecting an option replies ephemerally with the shared `buildEvidenceEmbed`; evidence is shown when present, while reason/raid/list/age/tracked-alts/logs remain usable without an image. The officer/senior-only "Added by" gate is unchanged. The row sits below Quick Add when both are present.
