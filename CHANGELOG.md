...
### Changed
- Gemini OCR for `/la-check` + auto-check now extracts the **class icon** for each name in addition to the name itself. Output shape moved from `string[]` to `Array<{name, ocrClass}>`; `filterAndDeduplicateEntries` + `extractNamesFromImage` updated accordingly. Backward-compatible: `checkNamesAgainstLists` still accepts legacy `string[]` input (older mocks, callers that don't OCR images themselves). The class string follows Lost Ark's English display names (Berserker, Bard, Souleater, etc.); Gemini is instructed to return `""` rather than guess when the icon is unreadable.
- `format.js` class-icon render now uses a priority chain: **snapshot > worker-meta probe > OCR**. Class icons stay visible on cold OCR checks (no snapshot AND worker offline) because the Gemini-read class falls through. Snapshot still wins when it has authoritative data; the meta probe (when worker online) still backfills snapshots for free. Support-class sorting honors the same priority chain so role grouping stays stable across cold and warm checks.
- OCR list-check + auto-check now run a targeted per-name `fetchCharacterMeta` probe for items lacking class+ilvl snapshot data. Partial revert of the earlier "DB-only after OCR" change: the broad prohibition stays for the heavy roster-page scrape, similar-name search, hidden-roster fallback, and Stronghold alt scan, but the single-name meta probe is restored because dropping it left a visible UX gap (no class icons for OCR'd names that had never been queried before). Cost is bounded: one bible call per previously-unseen name, results upserted to `RosterSnapshot` so subsequent OCR runs hit the snapshot for free. Routed via worker (`viaWorker: true`) so the call lands on the residential-IP host instead of Railway's CF-blocked egress. Concurrency from `LISTCHECK_ROSTER_LOOKUP_CONCURRENCY` (default 3), per-call timeout from `LISTCHECK_ROSTER_LOOKUP_TIMEOUT_MS` (default 6000ms). Per-name failure is non-fatal · the item just renders without class icon / ilvl as before. `listcheck-db-only.test.js` updated to keep the heavy-ops guards but allow targeted single-name meta enrichment.
- Worker-health gate added to the enrichment phase: before iterating per-name probes, `getWorkerHealth()` checks the residential-IP worker's heartbeat. If the worker is offline (stale-heartbeat or no record), the bot skips the entire enrichment phase with a single `[listcheck] Skipping meta enrichment ...` line and renders names without class/ilvl as before. Prevents the 400-600ms wasted latency + per-name log spam that would otherwise hit production when the operator's PC is off. Feature self-reactivates the next time a check runs after the worker is started.
