...
### Added
- Auto-check text input now documents and regression-tests both multi-name forms used by the roster team: `check NameOne NameTwo` and `check NameOne, NameTwo` (up to 8 unique valid names). Check result cards expose a **View details** dropdown for every blacklist/watchlist/whitelist hit, even when no evidence image was attached. The selected detail now uses the same headline + metadata-grid language as the list-change broadcast: Reason; Raid / Added / ilvl; then CP / officer-only Added by on one row; rich tracked alts; and the evidence image at the bottom. Snapshot enrichment on click is DB-only, so opening details never starts another Bible/worker request.
