...
### Fixed
- Auto-check / `/la-check` now canonicalizes over-accented OCR names from cached roster snapshots before rendering, e.g. `Aüréliá` -> `Aüreliá`. Snapshot metadata lookup uses diacritic-insensitive collation only for `RosterSnapshot`; blacklist / whitelist / watchlist / trusted matching stays stricter to avoid false list hits. 136/136 green.
