...
- Gemini OCR prompt repeated-letter rule extended to runs of 3+ identical glyphs (was "twice"). Trigger case: `Lpiiiv` (three i) came back as `Lpiiv` (two). Note this case is unrecoverable lookup-side · bible has real players at both `Lpiiiv` AND `Lpiiiiv`, so a collapsed `Lpiiv` is genuinely ambiguous and the bot cannot safely pick one; the prompt is the only safe lever and Quick-Add is the fallback when Gemini still miscounts.
