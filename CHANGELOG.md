...
### Fixed
- Rich `/la-list view` pages no longer slice the 4,096-character description in the middle of a custom emoji or roster link. The renderer preserves all ten entries and uniformly steps the alt preview down from three to two, one, or zero only when needed; an intact heads-only form is the final safety fallback. The same whole-token fitting protects the unpaginated Trusted view.
...
- `/la-list view` removes the per-entry evidence link from its compact rows because the evidence dropdown directly below already opens the complete detail card. Rehosted evidence is now resolved only when that dropdown is used, eliminating up to ten unnecessary Discord message fetches whenever a list page opens. The redundant `Page 1 / 44 · showing 10` line is gone too because the controls immediately below already show the current and total page.
