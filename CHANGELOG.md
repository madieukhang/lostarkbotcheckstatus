...
### Fixed
- Auto-check / `/la-check` no longer canonicalizes ambiguous same-base diacritic variants to Bible's first search result. Observed case: `Aürélia` / `Aürélià` lives in a crowded `Aurelia` search group where `Aüreliá` is the top result; the bot now requires an exact mark match when multiple real characters differ only by accents, and the OCR prompt explicitly calls out acute vs grave accents. 137/137 green.
