...
### Changed
- `/la-list view` now follows RaidManage's first-render pipeline: the list page appears before guild-name fetches or class decoration finish; those extras hydrate the visible page in the background and coalesce into the latest render. Page navigation reuses the command's list snapshot instead of reloading all collections after five seconds, while the explicit Refresh button still reloads them. Character and visible-alt rows gain class icons from one batched `RosterSnapshot` read, with no lostark.bible or ScraperAPI request.
