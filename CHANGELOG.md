...
- The cross-server card for a `/la-list edit` applied without approval shows the entry as saved: added alts appear in its roster and new evidence replaces the old image. It used to show the pre-edit roster and evidence while its change lines said both had changed. Edits that went through approval were already correct.
- Stronghold alt detection sends the hidden-roster item level lookup through the local worker when the scan does, like its other Bible requests. The lookup used to go out directly from Railway, whose IP Cloudflare blocks, so a hidden roster's item level came back empty.
- Screenshot checks try the next Gemini model when a reply finishes normally but holds malformed JSON, such as a trailing comma, as they already did for a reply without a JSON array or one cut off at the token limit. The check used to stop at the first model; "Gemini returned invalid JSON." now shows only when every model's reply was malformed.
...
### Changed
- `/la-reset` is registered only in the owner server and listed in `/la-help` there. The status it clears is shared by every server and decides when the owner channel gets its `@here` online notice, so a reset from any server just before the servers came back used to skip that notice. Other servers lose the command once the bot restarts and re-registers its commands.
- When a Discord shard has neither resumed nor re-identified ten minutes after it started reconnecting, the bot exits with code 1 so Railway restarts it, the same recovery the 60-second login watchdog already uses at startup. Shorter outages are left to discord.js, which resumes on its own. The reconnect logs also show how long each shard took to come back.
- The `/la-list edit` success card follows the `/la-list add` card: the title carries the list icon and character name, a hero line names who edited and what changed, and the List, Raid, Scope, Server, ilvl and CP fields, reason, tracked rosters, evidence download link and an **Edited by** footer match the add card. Changed values are marked in place with ✏️ and the old value struck through, and new alts carry 🆕, so nothing is shown twice. Roster stats come from stored snapshots, so an edit makes no Lost Ark Bible request.
...
- On a compact card the elapsed line speaks instead of labelling: *"Đọc xong trong 3.2s, ảnh này rõ nét ghê~"* for a fast read, *"Hết những 30.2s, xin lỗi để cậu chờ"* for a slow one. The uppercase `PHÂN TÍCH 30.2s` was borrowed from the full footer, where it has `// AN TOÀN` and `NGUỒN db…` as peers; alone under a sentence-case card that opens with "…cậu gửi nè" it read as a fragment of a different card, and a bare number at 30s is close to a complaint nobody acknowledges. Tone follows the wait through three `tPick` pools, the same shape the cleanup notice already uses for volume. Typed slash checks keep the terse label, because there it is among its own kind.
...
### Fixed
- The `/la-list add` duplicate warning now uses the empty sixth inline slot for **Server**. The flow reuses the roster it already loaded before duplicate detection, so this adds no additional Bible, database or ScraperAPI request; hidden/legacy results without a known Server retain the alignment spacer.
- Vietnamese `/la-check` roster previews now label the plural character row as `alts:` instead of the singular `alt:`.
- Rich `/la-list view` pages no longer slice the 4,096-character description in the middle of a custom emoji or roster link. The renderer preserves all eight entries and uniformly steps the alt preview down from three to two, one, or zero only when needed; an intact heads-only form is the final safety fallback. The same whole-token fitting protects the unpaginated Trusted view.
- List-check alt previews no longer lose their class icons on legacy list entries whose roster names exist but whose per-alt `RosterSnapshot` rows were never stored. The check first uses the existing batched snapshot read, then runs exact name-search only for the three alt links the card will actually display, with ScraperAPI explicitly disabled. Successful class/ilvl metadata is persisted under the roster entry, so the repair costs one lightweight pass and later checks return to the DB-only path.
- `/la-check` enrichment no longer throws away the server the roster page just reported. It reshaped each record before saving and kept only the name, class, ilvl and CP, so a page it had already paid for was scraped for nothing. This was the only free source of one: the name-search endpoint it falls back to answers with three columns (name, class, ilvl) and carries no server at all.

### Fixed
- The list-change broadcast could never show a server, even after it gained the field. Its stat map is built by `normalizeRosterStatRecord`, which rebuilds each record from a fixed list of five keys, so `world` was dropped no matter what fed it in - the cached snapshot, the caller's roster, or the self-healing fetch. A test that builds the map by hand does not exercise that normalizer, which is why the field looked correct in the suite and blank in Discord.

### Added
- The `/la-list add` success card shows **🌍 Server**. The add flow reads the roster page itself, so the server is already in hand when the card is built. A blacklist add reaches four inline fields (list, raid, scope, server) and pads out to whole rows; a whitelist or watchlist add has no scope field, so its three fit one row and stay unpadded.

### Changed
- The blacklist result card shown above `/la-roster` now swaps **CP** with **Added**, while **Server** keeps its existing place. Its inline grid reads `Raid / CP / ilvl` followed by `Added / Server`.
- `/la-list view` now shows eight characters per page instead of ten, leaving more room for class icons, linked names and alt previews before Discord's embed limit while keeping every row intact.
- `/la-list view` loads its list collections and compact `RosterSnapshot` projection in parallel, then sends one complete page instead of showing bare names and editing class icons in 1–2 seconds later. The production projection is about 250 KiB for 2,048 snapshots and overlaps the slower list read, so this removes the visual jump without restoring the old evidence-fetch bottleneck. Page navigation reuses that stat map; only explicit Refresh reloads it. No lostark.bible or ScraperAPI request is involved.
- `/la-list view` removes the per-entry evidence link from its compact rows because the evidence dropdown directly below already opens the complete detail card. Rehosted evidence is now resolved only when that dropdown is used, eliminating up to eight unnecessary Discord message fetches whenever a list page opens. The redundant page-summary line is gone too because the controls immediately below already show the current and total page.
- Typed `/la-list view` pages stop repeating their list icon before every character: the title and border already establish Blacklist, Whitelist, Watchlist, or Trusted. The marker remains on each row in `view all`, where it is necessary to distinguish entries from different lists; class icons remain in every view.
- An indirect list hit no longer repeats the character named by `via` in the alt preview beneath it. That character is already the link back to the owning list entry; removing it leaves all three preview slots for actual sibling alts and recalculates the `+N more` count from the filtered list.
- List-check results keep the reason and raid together on their detail line, then move an indirect `via` character onto its own following line. Direct list hits do not gain an empty or redundant `via` row, while long reports no longer bury the link back to the character that owns the list entry.
- The list-hit card `/la-roster` prints above a roster carries **🌍 Server** too, which squares its grid: it had four inline fields, so CP sat alone on a second row with nothing beside it. It deliberately stays at five and does not show Added by, because the searcher is being warned about the character, not told who filed the entry. `/la-roster` now stamps the server onto the characters it parses rather than only onto the rows it saves, which is what lets the card read it at all.
- The **🌍 Server** field reads across the whole roster instead of one character's own row. The server belongs to the roster rather than the character, so any sibling that has been scraped answers for all of them. That matters because an entry which already has class and ilvl never triggers another lookup, and would otherwise read "not available" forever no matter how many times it was checked. Applies to the `/la-check` detail card and the list-change broadcast, which already hold every sibling's snapshot to print their class and ilvl. A server found this way is rendered, never written back, so a wrong read costs one card rather than a stored row.
...
- Every character name on a list-check row now carries its class icon, not just the one that was searched. The entry a row matched through and the alts beneath it were the bare names sitting next to an icon-led name; the check service loads their snapshots in one extra query after the identity set is final. A legacy visible alt without a snapshot gets one bounded direct-search repair, while an unavailable or ambiguous name still degrades safely to a bare link.
...
- The evidence card (shared by `/la-roster` list hits, `/la-list view` detail, `/la-evidence`, `/la-search`, and the approval evidence button) now speaks the same grammar as the `/la-check` detail card. It takes an optional `statMap`, which turns its tracked-alt rows from bare links into class + ilvl + CP rows and adds 📊 ilvl / ⚔️ CP slots for the entry itself. 👤 Added by moves from the tail of the card into the inline grid beside CP, and the grid pads to whole three-column rows so a lone trailing field is never stretched to full width.
- `/la-roster` passes the roster it just fetched into that card, so a blacklist or whitelist hit is now as informative as the same entry seen from `/la-check`. The other four callers pass no `statMap` yet and render exactly as before: the stat slots are skipped rather than filled with "N/A".
- The `/la-roster` result card drops its footer. "Source: lostark.bible - run /la-roster again to refresh" restated the command the reader had just run, and the footer timestamp repeated the message timestamp Discord already prints next to the bot name.
- One code-wrap rule across every list card: fixed tokens and numbers (raid, ilvl, CP) render as `code`, while prose, links and timestamps stay plain. Raid was wrapped on the broadcast, check-detail, approval and add-success cards but not on the duplicate or `/la-list edit` success cards; CP was left plain on both grids where it sits directly beside a wrapped ilvl, even though the tracked-alt rows below it already wrapped the same value. The duplicate card's scope tag loses its backticks instead of gaining more: `[Toan cuc]` is already set apart by its brackets and its field label. Timestamps deliberately stay unwrapped, since backticks would print the raw `<t:UNIX:R>` rather than letting Discord render it in the viewer's locale.
- The two remaining `/la-list add` rejection cards pick up the same field icons. **Item level too low** gains 🎯 character, 📊 item level, 📉 minimum, 📒 target list, and two spacer slots: its fourth field used to sit alone on a second row where Discord stretched it to full width. **Roster not found** gains 🔍 on its suggestion list, and each suggestion now resolves its class through `getClassEmoji`, so the rows become icons on their own the day `CLASS_EMOJI_MAP` is filled in (every entry is currently empty).
- The "already in the list" card from `/la-list add` now labels every field with an icon, matching the success card the same command shows when a name does go in: 🔍 match type, 🧬 matched name, 🌐 scope, 📝 existing reason, 🗡️ raid. Only Added by / Time added carried icons before, so the two halves of the card read as different designs.
- Artist's copy no longer reads like system output with "I" bolted on. Roughly 105 strings across en/vi/jp were rewritten so the bot says things the way a person would: `OCR completed but found no valid character names` became `I read the image fine, but none of it looked like a character name`, and `Ignored 3 extra names (limit: 8)` became `I skipped 3 extra names - I can only take 8 at a time`. Engineer vocabulary (OCR, malformed, values, entry, row, database) is gone from the check, evidence, quick-add and broadcast surfaces, and the four broadcast headlines lose the one-template-with-the-verb-swapped shape. Every fact, placeholder and command reference is unchanged.
- Cleared the last third-person self-references: `dialogue.evidence` still said "LoaLogs may no longer have access to its storage channel" and cited "the LoaLogs evidence archive", which the earlier setup-voice pass had missed.
- The two list-check progress notices rotate between four phrasings each. They are the most-seen strings in the bot, one per screenshot check and one per typed check. The result-card footer tips are deliberately left alone: they are instructions, and instructions should stay stable so people learn them.


### Added
- Auto-check text input now documents and regression-tests both multi-name forms used by the roster team: `check NameOne NameTwo` and `check NameOne, NameTwo` (up to 8 unique valid names). Check result cards expose a **View details** dropdown for every blacklist/watchlist/whitelist hit, even when no evidence image was attached. The selected detail uses check-specific title/headline copy plus the broadcast metadata grid: Reason; Raid / Added / ilvl; then CP / officer-only Added by on one row; rich tracked alts; and the evidence image at the bottom. It describes the current list match instead of incorrectly announcing that the entry was just added. Snapshot enrichment on click is DB-only, so opening details never starts another Bible/worker request.
- Auto-check channel housekeeping. `/la-setup` gains a `repin` control and a guild-language control; `GuildConfig` now tracks the welcome pin and a daily cleanup cursor. The welcome embed introduces Artist as a passerby and explains the daily cleanup, fits Discord's embed limits in vi/en/jp, and a repin persists the fresh pin BEFORE deleting the tracked + orphan pins (rolling back the fresh message and keeping the old pins if the DB write fails). A daily cleanup scheduler then deletes every non-pinned message in that channel: it only touches guilds that have configured `autoCheckChannelId`, verifies the fetched channel belongs to that guild, skips pinned messages, claims each Vietnam day (00:00 Asia/Ho_Chi_Minh) exactly once via an atomic `findOneAndUpdate` so a repeated scheduler tick cannot double-delete, releases the day claim if any deletion fails, and runs on a single overlap-safe timer.
- Evidence broadcasts carry a compact refreshable button for rehosted images (legacy direct URLs keep a plain link fallback). The handler acknowledges before refreshing, returns the full image privately, and says so plainly when the archived evidence is gone.
- `utils/artistVoice.js`: the shared alert embeds now carry the Artist identity without overwriting operational footers.
- `services/roster/rosterSnapshots.js`: `upsertRosterSnapshots` writes one normalized bulk update per roster character, and is a no-op for an empty roster.
...
### Changed
- Screenshot list-check latency is now bounded by the existing 8-name / 3-worker limits: ambiguity lookups run with bounded concurrency, duplicate name/prefix searches share one request-local in-flight cache across OCR and enrichment, and successful metadata snapshots persist in one bulk write instead of one write per name. A deterministic 60ms-per-search benchmark reduced the 8-name refine path from 573ms to 224ms and the valid-empty recovery path from 15 requests / 1058ms to 8 requests / 557ms. Overflow OCR names remain available for the existing ignored-count UI.
- Welcome pin + setup notices presentation pass. The pinned auto-check guide grows from 4 to 6 fields: a four-list glossary (blacklist/watchlist/whitelist/trusted), a new `🌐 Global vs server` scope field (blacklist entries are global = shared across every server using the bot, or server-only; whitelist/watchlist/trusted are always server-scoped), and an officer-tools field (Quick Add, `/la-evidence public`, `/la-roster deep`). The pin title stays byte-stable so `isOwnedWelcome` pin detection keeps matching (vi/en/jp). Every `/la-setup` confirmation notice now speaks in Artist's first person (drops third-person "LoaLogs will/has...") and points at the consolidated `config action:<choice>` syntax instead of the dead `/la-setup <subcommand>` forms; the `/la-help` Server Setup group and the README command table are rewritten to match. The check-result card sources its color from the shared `COLORS` tokens instead of duplicated hexes (no visual change). Embed identity stays Minimal: `decorateArtistEmbed` remains an intentional no-op, so the polish is copy + consistency, not a repeated badge. Guard tests cover the pin structure, the check-card colors, and the setup-notice voice + live command refs. 233 tests green.
- /la-setup is now one action command instead of eight subcommands: `/la-setup config action:<show|set-auto-channel|set-notify-channel|set-language|set-default-scope|cleanup-on|cleanup-off|notify-on|notify-off|repin> [channel] [language] [scope]`. `action:show` is the status hub (channels + source, global/server default scope, notifications on/off, daily cleanup on/off + last run, language, pin jump). The action autocomplete only offers the toggle that changes state (once cleanup is on, only cleanup-off shows), so the picker itself signals current state, and every toggle reply points back to action:show. The eight old subcommands (/la-setup autochannel, ...) are removed; slash re-registration drops them on deploy.
- Artist now speaks as the host of the auto-check channel. The pinned welcome no longer casts Artist as a passerby who "left a sign behind" while narrating the bot as "LoaLogs" in the third person; Artist greets the channel, explains the check flow in first person (`I react with 🔍`, `I delete every non-pinned message`), and keeps the same embed anatomy and every fact (8 names, four list types, Quick Add, the 00:00 Asia/Ho_Chi_Minh sweep, pinned survival, the command list). The same first-person rule is swept across the `/la-language-switch` copy: thirteen strings in vi/en/jp, `{flag}` and `{label}` preserved. Copy only - `utils/artistVoice.js` stays an empty seam and no production code changed. A regression guard now asserts the welcome embed speaks in first person and contains no third-person "LoaLogs" in any language. 211 tests green.
- List models consolidated onto one shared document shape: new `models/listEntrySchema.js` exports `createListEntrySchema({ scoped })`, and Blacklist / Whitelist / Watchlist now build from it instead of each redeclaring the same 15 fields. Index specs are preserved exactly (Blacklist keeps its compound `{name, scope, guildId}` unique index with the `en` / strength-2 collation, the other two keep their name-only unique index, and all three keep `{allCharacters: 1}`), so no Mongo index migration is needed. `broadcasts.js` likewise extracts `sendEmbedToChannels()` in place of three duplicated fetch/send/catch loops, and `listEntryMap.js` absorbs the shared entry lookup. Net -273 lines. 161 tests green.
...
- `/la-stats`: bare author line replaced by a `📊 Bot Statistics` title + one-line hero; the refresh hint folds into the footer (`Officer-only · ephemeral · re-run /la-stats to refresh`).
- Multiadd: preview title carries the row count (`📋 Bulk Add Preview · 12 rows`); bulk summary title carries the outcome (`📋 Bulk Add Complete · 12/15`), the redundant bare-number Added/Skipped/Failed 3-up is dropped (each detail field header already shows its count, now with ✅/⚠️/❌ icons), and the footer gains a `verify with /la-list view` tip. 151/151 green.
...
### Fixed
- Managed welcome lifecycle now enforces the Discord guild boundary twice: the target setup channel must belong to the requesting guild, and a stale welcome stored in Mongo is deleted only after its source channel reports the same guild ID. A corrupted or manually edited cross-guild channel reference is rejected before cleanup/send, while a cross-guild stale reference is left untouched.
- Lostark.bible name search now accepts both the legacy `result` envelope and the live `data` reference-table envelope (including its `{ _: rootIndex }` wrapper). Valid names no longer collapse to empty results and trigger repeated prefix-recovery calls in `check <name>`, screenshot checks, `/la-search`, or hidden-roster fallbacks. Unsupported response shapes now fail fast instead of fanning out more search requests. Added live-contract, legacy-compatibility, empty-result, and no-fan-out regressions; 238/238 tests green.
...
- Auto-check / `/la-check` now applies a party item-level guard after OCR/image refinement: if an exact unmarked name is a low outlier but a marked same-base sibling fits the lobby median, the row is corrected before formatting. Covers the live `Cruelfighter` 1640 vs `Crüelfighter` 1768 lobby without changing legitimate low-level `Cruelfighter` groups. 142/142 green.
- Auto-check / `/la-check` now runs a targeted second OCR pass when the first pass returns an exact unmarked name that has marked Bible siblings, e.g. `Cruelfighter` vs `Crüelfighter`. The second pass reuses the original image and forces Gemini to choose only from the concrete candidate list, so visible umlauts can be restored before list-check enrichment picks the wrong exact unmarked character. 140/140 green.
- Auto-check / `/la-check` no longer canonicalizes ambiguous same-base diacritic variants to Bible's first search result. Observed case: `Aürélia` / `Aürélià` lives in a crowded `Aurelia` search group where `Aüreliá` is the top result; the bot now accepts exact or uniquely-nearest accent patterns, but refuses tied accent guesses. This preserves `Crüelfighter` / `Qiylyn` enrichment while still blocking unsafe `Aurelia` guesses. OCR prompt also calls out acute vs grave accents. 139/139 green.
...
- Comment-spec batch E applied to `bot/services/worker/*` (2) + `bot/services/multiadd/*` (4). Worker side: heartbeat (start/get/stop) + scrape-worker (claim/process loop) headers now state the lease-window + stale-recovery invariant in one place. Multiadd: parser headers callout the dynamic header-row scan, template + index pick up file-spec headers. 129/129 green across two consecutive runs.
- Comment-spec batch D applied to `bot/services/roster/*` (7 files: altDetection, workerBibleClient, bibleClient, bibleFetch, listChecks, buildRosterCharacters, search). Each picked up the standardized file-spec header and JSDoc on the exported surface (`@param`/`@returns` per the workspace spec). `search.js` header links the bible-API behaviour memory so future contributors find the NFC-only + single-edit-tolerance rules. No behaviour change, 129/129 green across two consecutive runs.
- OCR list-check + auto-check enrichment now routes by worker health instead of going dark when the worker is offline. Two cases: **worker online** → `buildRosterCharacters` via worker (single roster-page scrape returns class + ilvl + CP + the full alt list); **worker offline** → `fetchNameSuggestions` direct from Railway (bible's lightweight search endpoint, less aggressive CF protection than the per-character HTML page; class + ilvl only, no alts). Both paths persist class+ilvl+CP to `RosterSnapshot` so subsequent OCR runs hit cache for free. Concurrency from `LISTCHECK_ROSTER_LOOKUP_CONCURRENCY` (default 3); per-name failure is non-fatal · the formatter renders bare name. Replaces the previous "skip entire enrichment when worker offline" behavior · class icons + ilvl now surface in both modes whenever bible responds.
- `listcheck-db-only.test.js` relaxed once more: `buildRosterCharacters` is now ALLOWED in the service module because it backs the worker-online enrichment branch (single per-name scrape, bounded by `LISTCHECK_MAX_NAMES` and the concurrency knob; we still avoid hidden-roster fallback / Stronghold alt scan inside it). Other heavy-ops guards (RosterCache, hidden-roster fallback, post-check enrichment queue) stay in place.
- Diacritic-tolerant fallback in the offline branch: Gemini OCR occasionally strips combining marks (observed: "Trùmfighter" returned as "Trumfighter" despite the prompt asking to preserve diacritics). When the case-insensitive exact match against bible's search results fails, the bot retries with combining marks stripped on both sides (NFD + drop `̀-ͯ` + lowercase). On a hit, `item.name` is updated to the canonical (with-diacritic) form so the embed renders the correct spelling and the snapshot is keyed by the real name. Logged as `[listcheck] Diacritic-tolerant match: OCR'd "X" -> canonical "Y"` for forensics.
- Edit-distance fuzzy match as a third pass when exact + diacritic-tolerant both fail. Catches Gemini OCR mistakes beyond diacritic loss: doubled letters (observed: "Trùmfighter" returned as "Trùmffighter"), missing letters, swapped look-alikes (l vs I, 0 vs O). Levenshtein distance on canonical (diacritic-stripped) forms; threshold scales with name length to avoid false positives on short names: `<6 chars` no fuzzy, `6-11 chars ≤1 edit`, `12+ chars ≤2 edits`. Bible has already narrowed to similar names via its search index, so the smallest-distance candidate within threshold is almost certainly the real character. Picks the closest suggestion; ties break on bible's relevance ordering. Updates `item.name` to bible's canonical form on a hit. Logged as `[listcheck] Fuzzy match (edit dist N <= M): OCR'd "X" -> canonical "Y"`.
- Gemini OCR prompt tightened with explicit letter-count preservation rule (don't double or collapse letters) and look-alike disambiguation (lowercase L vs uppercase I vs digit 1; digit 0 vs uppercase O). Reduces the rate at which the diacritic-tolerant + fuzzy fallbacks have to kick in.
UX rework pass:
- `/la-list add` success embed rebuilt around the same vocabulary as the approval-DM card so officers see one consistent product across the add flow. Title moves to `<List> · Added · <Name>` with done + list-type emoji prefix (whitelist drops the done prefix so the title doesn't double-tick). New hero line in plain English ("X added Y to the blacklist [Global]."), inline meta row (📒 List · 🗡️ Raid · 🌐 Scope · blacklist-only), full-width 📝 Reason, numbered 🧬 Tracked alts with class icon + ilvl from the roster scrape (cap 12 visible, overflow tail), 🔗 Links row with [Roster] · [Logs] · [Evidence Logs] when present, footer crediting the requester + roster source. Image (fresh resolved evidence URL) unchanged.
- `/la-setup view` rebuilt from a single bullet-list description into a 2×2 inline-field grid (auto-check channel · notify channel · default scope · global-notify toggle), each cell pairing the value with an italic source qualifier (`set via /la-setup` vs `from env var fallback` vs `Not configured`). Guild name moves to the embed author header; footer credits the last admin who touched the config or notes there's no persisted config yet.
- Server-online monitor notification rebuilt. Title moved from generic "Thông báo" to `🟢 <Server> is back online`; description "Logins are open · Time to raid!"; inline meta fields (🌐 Server · 🕐 Came online relative time · 📡 Also monitored when there are other configured targets); footer `Source: playlostark.com · /la-status for live state`. Uses `COLORS.success` instead of the hardcoded magic color, matches the English-first LoaLogs voice the rest of the bot follows.
- Approval-flow evidence button (`list/add/evidenceButton.js`) now routes through the shared `buildEvidenceEmbed` renderer so approvers see the same rich card (Reason field · inline meta · Tracked alts · image) that `/la-evidence`, `/la-search`, and `/la-list view` use. The thinner inline embed it used before only carried name + reason + image; the consistent shape closes a UX gap where reviewing evidence in the approval DM looked different from the evidence-focused surfaces.
...
- `listcheck-db-only.test.js` relaxed further: `fetchNameSuggestions` is now ALLOWED in the service module because it backs the worker-offline branch. Heavy-ops guards (`buildRosterCharacters`, `RosterCache`, `hiddenRosterFallback`, `queueFlaggedListEntryEnrichment`, etc.) stay in place.

### Fixed (review findings against b5db9ce)
- **Crash**: `/la-roster` hidden-roster branch threw `ReferenceError: bibleProfileUrl is not defined` (and similarly for `rosterUrl` further down the same handler). The Phase A.1 swap had landed at both call sites in `bot/handlers/roster/hiddenRoster.js` (line 185 and line 202) but the matching `import { rosterUrl, profileUrl as bibleProfileUrl } from '../../utils/rosterLink.js'` line never made it onto the file because an earlier batch Edit's `old_string` mis-matched the two-import-per-line shape and the tool surfaced the error mid-stream. Import is now present; auditing the other 16 swapped files confirmed no other missing-import case.
- **Scope leak**: `/la-evidence` did not apply the blacklist scope filter, so a user in Guild A could surface a server-scoped Blacklist entry belonging to Guild B via autocomplete or free-typed name. Autocomplete now routes blacklist queries through `buildBlacklistScopeFilter`; the command-handler lookup does the same; whitelist/watchlist (no scope concept) pass through unchanged. Owner guild still sees everything.
- **Encoding ambiguity**: `/la-evidence` autocomplete encoded the option value as `<type>:<name>`, which is ambiguous when the same name lives under both global and server scope. Value is now `<type>:<_id>` (24-char Mongo ObjectId) so the handler resolves unambiguously even when scope/cross-list overlap exists. Free-typed input still works via priority lookup. Autocomplete label gets a `[S]` tag on server-scoped blacklist rows.
- **Coverage gap**: `/la-roster` embed `.setURL(targetUrl)` in `roster/command.js` reused the `targetUrl` variable that `bibleClient.fetch` consumes, so even after Phase A.1 the embed title still pointed at `lostark.bible` regardless of `BIBLE_BASE_URL`. Embed now uses `rosterUrl(name)` for display; `targetUrl` stays hardcoded for the fetch call only (data-fetch URLs are intentionally controlled by the scraper/worker layer, not by `bibleBaseUrl`).
- **Enrichment-metadata coverage gap**: the cross-list move via approval (`editApproval.js`) and the dupe-overwrite path (`overwriteButton.js`) wrote `allCharacters` without stamping `enrichmentSource`/`enrichedAt`, so a future stale-loop would have treated freshly-moved or freshly-refreshed entries as legacy null. `editApproval.js` now preserves the metadata from the pre-move entry; `overwriteButton.js` stamps `source='bible', enrichedAt=now()` whenever the roster refresh succeeded. Matches the pattern already in `applyNow.js`.

### Added
- Roster alts line on every `/la-check` / auto-check result row when alts are known. Priority chain mirrors the renderer's other layers: **DB list entry's `allCharacters`** (free, used when the row hit blacklist / whitelist / watchlist / trusted) > **`item.discoveredAlts`** (bible roster fetch from the online enrichment branch, only when `rosterVisibility === 'visible'`). Capped at 3 visible names with `*+N more*` overflow so the 4096-char description budget stays in bounds. Skipped silently when the roster is hidden or missing, or when the worker is offline and bible's lightweight search endpoint can't surface alts. Format: `↳ alts: A, B, C +N more`.
- Details dropdown on `/la-check` slash responses and auto-check listener replies. It surfaces every blacklist/whitelist/watchlist hit, including entries without an attached image, and de-duplicates multiple checked alts that resolve to the same entry. Options keep the `<listType>:<_id>` encoding so the select handler resolves unambiguously across types and scopes (same encoding as `/la-evidence`). Selecting an option replies ephemerally with a check-specific broadcast-style card: reason; raid/age/ilvl; CP plus the officer/senior-only Added by field; tracked alts enriched from cached snapshots; and the evidence image below. The row sits below Quick Add when both are present.
...
- New `enrichmentSource` (enum: `bible` / `manual` / `local-sync` / `null`) and `enrichedAt` (Date | null) fields on Blacklist / Whitelist / Watchlist schemas. Tags where each entry's `allCharacters` was last touched from so future re-enrich-when-stale loops can reason about source + age without coupling renderers to a specific upstream. Additive only · legacy entries keep null for both fields and stay queryable as before. Write paths now stamp the fields: `/la-list add` (source=bible, now), `/la-list edit additional_names:...` (source=manual, now, both for in-place and type-move), `/la-list enrich` save (source=bible, now).
...
### Added
- Prefix-indel recovery: a final search fallback for OCR names where a single letter was inserted or dropped (often a miscounted repeated letter · `Qiyllyn` for `Qiylyn`, `Lpiiv` for `Lpiiiv`). bible's search is prefix-based and returns `[[]]` for the typo'd full name, so the bot re-queries the 4-char prefix and keeps only candidates that are exactly one indel away: length differs by exactly 1, Levenshtein distance is exactly 1, and a subsequence relationship holds. The subsequence gate is what makes it safe · it rejects same-length substitutions like `Viatchu` -> `Viatchy` (a real but different player) that a plain distance-1 fuzzy would wrongly accept and misattribute class/ilvl onto. Accepts only when exactly one candidate survives; true ambiguity renders bare. Runs only after exact + ASCII-fold + the 4-pass canonical matcher all miss. Logged as `[listcheck] Prefix-indel recovery: OCR'd "X" -> canonical "Y"`.

### Fixed
- Auto-check + `/la-check` now resolves trust via the same roster scrape it already pays for during worker-online enrichment. Phase 2 of the trust resolver previously walked `allCharacters` from blacklist / whitelist / watchlist entries only, so a char whose bible roster shared a main with a `TrustedUser` entry but had no DB list hit (e.g. Morrahduk on Clauseduk's roster, Clauseduk trusted) rendered as plain `❓ not listed`. The resolver now also walks `item.discoveredAlts` (populated by the worker-online roster page scrape) and matches those alt names against `TrustedUser`. No extra bible call · the alt list is already in hand. Limitation: when the worker is offline, search-direct does not populate `discoveredAlts`, so trust-via-roster does not fire on those rows (matches the existing trust-via-blacklist contract that needs alts populated at add-time).

### Changed
- Worker-online enrichment now canonicalizes the display name to bible's exact spelling when the roster scrape resolves the target (mirrors the search-direct branch, which already did this). Previously a row resolved via the worker roster page kept the OCR'd casing/diacritics, so e.g. `Aūréliá` (OCR macron) rendered as-typed even though bible's character is `Aüreliá` and the lookup succeeded (correct class + ilvl + CP). The snapshot is now keyed by the canonical name too, so the next check on the same player hits cache under bible's spelling.
- Gemini OCR prompt repeated-letter rule extended to runs of 3+ identical glyphs (was "twice"). Trigger case: `Lpiiiv` (three i) came back as `Lpiiv` (two). (Superseded note: an earlier version of this entry called `Lpiiv` unrecoverable lookup-side on the theory that `Lpiiiv` and `Lpiiiiv` were equally plausible. That was wrong · they are NOT equidistant from `Lpiiv` (`Lpiiiv` = 1 insertion, `Lpiiiiv` = 2), so the prefix-indel recovery below resolves it unambiguously. The prompt rule still helps reduce how often recovery has to run.)
- Gemini OCR prompt gains an explicit rule for lowercase Latin look-alikes that lobby fonts blur (a vs e, a vs o, c vs e, u vs v, rn vs m). Earlier rules only covered ascender/digit confusions (l vs I vs 1, 0 vs O) and diacritic preservation, so substitutions like `Vietchu` returned as `Viatchu` slipped through and rendered bare because bible's search is prefix-based · the canonical row sits behind a different prefix than the OCR'd query. No lookup-side recovery is safe (a prefix-retry to `Viat*` returns `Viatchy`, a real but different player, and a fuzzy match would silently misattribute snap class + ilvl + CP). Tightening the upstream prompt is the only no-false-positive lever. Recovery still falls to the Quick-Add dropdown when Gemini misses anyway.
- Trust icon migrated from `💚` (heart) to `🛡️` (shield) everywhere it surfaces in `/la-check` / auto-check output: per-row primary line (Trusted-only result), per-row tag suffix (when also flagged on another list), and the embed header summary count. Aligns with the `/la-list trust action:add` and `/la-list trust action:remove` success cards (already `🛡️ Trusted · Added / Removed` in title) so the trust vocabulary is finally consistent across every surface.
...
- Added a third case: worker-online roster scrape returns a two-char roster (Morrahduk + Clauseduk), `Clauseduk` is seeded in `TrustedUser`, and `Morrahduk` correctly inherits trust via `item.discoveredAlts`. Also asserts the rendered line uses the new `🛡️` icon and no longer carries the legacy `💚`.
