...
### Changed
- OCR list-check + auto-check enrichment now routes by worker health instead of going dark when the worker is offline. Two cases: **worker online** → `fetchCharacterMeta` via worker (full meta on residential IP, as before); **worker offline** → `fetchNameSuggestions` direct from Railway (bible's lightweight search endpoint, less aggressive CF protection than the per-character HTML page). Both paths persist class+ilvl to `RosterSnapshot` so subsequent OCR runs hit cache for free. Concurrency from `LISTCHECK_ROSTER_LOOKUP_CONCURRENCY` (default 3); per-name failure is non-fatal · the formatter renders bare name. Replaces the previous "skip entire enrichment when worker offline" behavior · class icons + ilvl now surface in both modes whenever bible responds.
