...
const jp = {
  ...
  autoCheckWelcome: {
    ...
    listsName: '📚 4つの list',
    listsValue: [
      '⛔ **Blacklist** - 要注意の player ですわ。',
      '⚠️ **Watchlist** - 未確定ですが気に留めておきたい方ですの。',
      '✅ **Whitelist** - 信頼できる既知の方ですわ。',
      '🛡️ **Trusted** - 検証済みの trusted record ですの。',
      'Black と watch は要注意、white と trusted は照合 context として表示されますわ。',
    ],
    scopeName: '🌐 Global と server の list',
    scopeValue: [
    ...
    cleanupDisabledValue: [
      ...
      'Manage Server 権限を持つ admin は `/la-setup config action:cleanup-on` で有効化できますの。',
    ],
    quickAddName: '⚡ Quick Add と officer 用ツール',
    quickAddValue: [
      '• **Quick Add** - check の後、officer は dropdown から未登録の名前を list へ登録できますわ。',
      '• `/la-evidence name:<character> public:true` - evidence を公開投稿 (officer/senior)。',
      '• `/la-roster name:<character> deep:true` - Stronghold の alt scan (officer/senior)。',
    ],
