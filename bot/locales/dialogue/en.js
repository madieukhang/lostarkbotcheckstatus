export default {
  common: {
    ...
    wrongTextChannel: {
      ...
      description: 'Please choose a **text channel** so I can read and reply there.',
    },
    missingPermissions: {
      title: 'A few permissions are missing',
      description: 'I cannot finish setting up <#{channel}> until I have the permissions below.',
  ...
  setup: {
    ...
    welcomeCreateFailed: 'I could not pin a new welcome guide, and there was no previous Artist guide to keep.',
    ...
    actions: {
      ...
      setNotifyChannel: 'Set notification channel',
      setLanguage: 'Set public language',
  ...
  broadcast: {
    ...
    headlines: {
      added: '{icon} {name} just went onto the **{list}**{scope}. Everything worth knowing is below.',
      removed: '{icon} {name} came off the **{list}**{scope}. Here is what the entry held.',
      edited: '{icon} Someone revised {name} on the **{list}**{scope}. The updated details are below.',
      enriched: '{icon} I turned up **{newCount}** more {altWord} for {name} on the **{list}**{scope} - that makes **{total}** tracked.',
    },
    altOne: 'alt',
    altMany: 'alts',
  ...
  approval: {
    ...
    flow: {
      ...
      compareRaid: 'Raid', compareAdded: 'Added', compareBy: 'By',
  ...
  listEdit: {
    ...
    success: { titleMoved: '{list} · Edited & moved · {name}', titleEdited: '{list} · Edited · {name}', changes: 'Changes ({count})', hero: '**{user}** edited {name} in **{list}**{scope}{summary}.', summaryChanged: ' · changed {fields}', summaryAlts: ' · added **{count}** alts', summaryFields: { logs: 'Logs', evidence: 'Evidence' }, footer: 'Edited by {user}' },
    evidence: { before: 'Before the edit', after: 'After the edit', none: 'No previous image', unavailable: 'Image unavailable', download: '[Download original]({url})' },
    command: {
  ...
  remove: {
    ...
    line: { variants: [
      'Removed {name}. I copied the old reason below, in case you need it again.',
      'Done, {name} is off the list. Its reason is kept right here.',
      'Removed {name}. Nothing about that entry is left in the database, so here is a copy.',
    ] },
    lineNoReason: { variants: [
      'Removed {name}. That entry carried no reason, so there is nothing to keep.',
      'Done, {name} is off the list.',
    ] },
    removedBy: 'Removed by', removedAt: 'Removed',
  ...
  check: {
    ...
    }, imageQueued: 'Screenshot check queued · **{count}** image request(s) ahead…', imageProgress: 'Images received · reading **{current}/{count}**…', imageCooldownWait: 'Gemini is cooling down after an API error · retrying image **{current}/{count}** in **{seconds}s**…', imageIgnored: 'Each check handles up to {limit} images, so I skipped **{count}** extra.', partialImages: { title: 'Some images could not be read', description: 'I skipped **{failed}/{count}** image(s) after OCR errors. The result above uses only the images that succeeded.', footer: 'You can post the failed image again on its own.' }, ignored: 'I skipped **{count}** extra {word} - I can only take {limit} at a time.', nameOne: 'name', nameMany: 'names',
    ...
    embed: {
      ...
      flagged: 'FLAGGED {count}', clear: 'CLEAR', elapsed: '⏱️ TOOK {seconds}s', elapsedQuick: { variants: [
        ...
        '⏱️ {seconds}s. Worth the extra look.',
      ] }, elapsedSlow: { variants: [
        '⏱️ {seconds}s, sorry for the wait. Plenty of names to read on that one.',
        '⏱️ {seconds}s. I would rather read slowly than guess a name wrong.',
