export default {
  common: {
    ...
    wrongTextChannel: { title: 'そのチャンネルは使えませんの', description: 'わたくしが読んで返事できる **テキストチャンネル** を選んでくださいませ。' },
    missingPermissions: { title: '権限が少し足りませんわ', description: '<#{channel}> の設定を終えるには、わたくしに次の権限が必要ですの。', footer: 'チャンネル権限を直してから、もう一度お試しくださいませ。' },
  },
  system: {
    status: {
  ...
  setup: {
    ...
    welcomeCreateFailed: '新しい welcome guide をピン留めできず、残しておける以前の Artist ガイドもありませんでしたの。',
    ...
    actions: {
      ...
      setNotifyChannel: '通知 channel を設定',
      setLanguage: '公開言語を設定',
  ...
  check: {
    ...
    text: { empty: { title: '肝心のお名前が抜けておりますわ〜', description: 'ここで `check <character>` か `check: <character>` と入力してくだされば、お調べいたしますわ〜', footer: '何名でもかまいませんの。スペース、カンマ、改行で区切ってくださいませ。' }, invalid: { title: 'そちらは名前には見えませんの', description: 'こちらは Lost Ark の character 名として読み取れませんでしたわ: {tokens}', footer: '`check` のあとは character 名だけにしてくださいませ。link と mention は読み飛ばしますの。' }, progress: {
      variants: [
          ...
          '**{count}** {word}を受け取りましたわ · 今お調べいたしますの…',
          '**{count}** {word}を list と突き合わせておりますわ…',
        ],
    } },
    progress: {
      variants: [
          '**{count}** {word}を抽出 · database list を確認中ですの…',
          '画像から **{count}** {word}を読み取りましたわ · list を照合中ですの…',
    ...
    embed: { imageAuthor: '送ってくれた画像から読み取ったリストだよ。', autoKicker: 'AUTO-CHECK', slashKicker: 'LIST CHECK', names: 'NAMES', notListed: '未登録', configured: 'configured', ignored: '上限 {limit} のため {count} {word}を除外しましたの。', flagged: 'FLAGGED {count}', clear: 'CLEAR', correctedOcr: 'OCR 補正 {count} 名 · screenshot と照合', correctedText: '入力名を {count} 名補正 · 元の入力と照合', quickFlagged: 'Dropdown で未登録名を Quick Add · /la-roster <name> で詳細', quickClean: '下の dropdown で未登録名を Quick Add', rosterTip: '/la-roster <name> で flagged hit の full roster', rerunTip: 'Fresh image で再実行してくださいませ', source: 'SRC db blacklist + whitelist + watchlist + trusted' },
    format: { via: '**{name}** 経由', correctedOcr: 'OCR **{input}** → lostark.bible **{name}**', correctedText: '入力 **{input}** → lostark.bible **{name}**', rosterVia: 'roster alt **{name}**', rosterEntry: 'roster alt **{name}** → entry **{entry}**', sameRoster: '同じ roster の {count} 名', inImage: '画像内', alts: 'alts', more: '+{count} 名', local: 'サーバー内', trusted: 'trusted' },
  },
  scan: {
