...
function buildOverviewLines(lang, isOwnerGuild) {
  const lines = lang === 'vn' ? [
    ...
    '',
    '`/la-roster tên [deep] [deep_limit]` · Tra cứu roster + theo dõi ilvl. `deep:true` quét alt qua Stronghold (**chỉ officer/senior** vì cần worker residential-IP)',
    '`/la-search tên [min_ilvl] [max_ilvl] [class]` · Tìm tên tương tự với bộ lọc',
    '`/la-evidence tên [public]` · 📎 Xem trực tiếp evidence của 1 entry (autocomplete từ DB, không qua list view). `public:true` chỉ officer/senior',
    '',
    ...
    '',
    '`/la-roster name [deep] [deep_limit]` · Fetch roster + progression tracking + list check. `deep:true` runs Stronghold alt scan (**officers/seniors only** since it needs the residential-IP worker)',
