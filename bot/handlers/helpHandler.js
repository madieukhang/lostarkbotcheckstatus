...
export async function handleHelpCommand(interaction) {
  ...
  const helpLines = lang === 'vn' ? [
    ...
    '',
    '`/la-roster tên [deep] [deep_limit]` · Tra cứu roster + theo dõi ilvl. `deep:true` quét alt qua Stronghold; user thường chỉ chạy 1 scan cùng lúc',
    ...
    '`/la-list enrich tên [deep_limit]` · 🔍 Stronghold deep-scan entry đã có và append alts khám phá được (user thường 1 scan cùng lúc, ~10-15 phút)',
    ...
    '',
    '`/la-roster name [deep] [deep_limit]` · Fetch roster + progression tracking + list check. `deep:true` runs Stronghold alt scan; regular users can run one scan at a time',
    ...
    '`/la-list enrich name [deep_limit]` · 🔍 Stronghold deep-scan an existing entry and append discovered alts (regular users: one scan at a time, ~10-15 min)',
