...
| `action:cleanup-on` / `cleanup-off` | Toggle daily cleanup for the auto-check channel |
...
| `GEMINI_MODELS` | `gemini-3.8-flash,gemini-3.7-flash,gemini-3.6-flash,gemini-3.5-flash,gemini-3.5-flash-lite,gemini-3.1-flash-lite` | Gemini 3.x-only priority list for OCR failover; older generations are ignored |
| `GEMINI_PRIMARY_TIMEOUT_MS` | `12000` | Soft timeout for the preferred OCR model; fallbacks keep the remainder of the shared 30s budget |
